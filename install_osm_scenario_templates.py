...
# @file    install_osm_scenario_templates.py
# @author  Marek Heinrich
# @author  Michael Behrisch
# @date    2020-03-25

# Modified version of "install_scenario_templates.py" to work with OSM nets
...
def create_template_folder(scenario_name, options):
    ...
    if not options.no_network:
        # check for osm folder and import data
        osm_dir = os.path.join(scenario_pre_dir, 'osm')
        if os.path.isfile(osm_dir): # TODO what does?
            # emulate symlink
            osm_dir = os.path.join(options.pre, open(osm_dir).read().strip())
    ...
    if os.path.isdir(shapes_dir):
        ...
        if options.suburb_taz:
            ...
            if not os.path.exists(tazFile) or os.path.getmtime(tazFile) < os.path.getmtime(net_path):
                ...
                if net is None:
                    net = sumolib.net.readNet(net_path, withConnections=False, withFoes=False)
                get_germany_taz.get_polys(options, net=net)
